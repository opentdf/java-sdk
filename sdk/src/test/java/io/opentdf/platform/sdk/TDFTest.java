...
public class TDFTest {
        ...
        when(kasRegistryService.listKeyAccessServersBlocking(any(ListKeyAccessServersRequest.class), any()))
                .thenReturn(new UnaryBlockingCall<>() {
                    ...
                    public ResponseMessage<ListKeyAccessServersResponse> execute() {
                        return new ResponseMessage.Success<>(mockResponse, Collections.emptyMap(),
    ...
    void testSimpleTDFEncryptAndDecrypt() throws Exception {
        ...
        List<TDFConfigPair> tdfConfigPairs = List.of(
                new TDFConfigPair(
                        Config.newTDFConfig(Config.withAutoconfigure(false),
                                ...
                                Config.withDataAttributes("https://example.org/attr/a/value/b",
        ...
        for (TDFConfigPair configPair : tdfConfigPairs) {
            ...
            var size = manifest.encryptionInformation.integrityInformation.segments.stream().map(s -> s.segmentSize)
                    .reduce(0L, Long::sum);
            ...
            assertThat(policyObject.body.dataAttributes.stream().map(a -> a.attribute).collect(Collectors.toList()))
                    .asList()
    ...
    void testSimpleTDFWithAssertionWithRS256() throws Exception {
        ...
        TDF tdf = new TDF(
                new FakeServicesBuilder().setKas(kas).setKeyAccessServerRegistryService(kasRegistryService).build());
    ...
    void testKasAllowlist() throws Exception {
        ...
        try {
            tdf.loadTDF(new SeekableInMemoryByteChannel(tdfOutputStream.toByteArray()), Config.newTDFReaderConfig(),
                    platformUrl);
        ...
        Reader reader = tdf.loadTDF(new SeekableInMemoryByteChannel(tdfOutputStream.toByteArray()), readerConfig,
                platformUrl);
        reader.readPayload(unwrappedData);
        ...
        tdf = new TDF(new FakeServicesBuilder().setKas(kas).setKeyAccessServerRegistryService(kasRegistryServiceNoUrl)
                .build());
        tdf.createTDF(plainTextInputStream, tdfOutputStream, config);
